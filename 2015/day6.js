...
// input = ['turn on 0,0 through 1,1']

{ // Part 1
  const lights = Array(1000*1000).fill(false)

  for (const command of input) {
    const [action, fromXY, toXY] = command.split(/(toggle|turn off|turn on) (\d{1,3},\d{1,3}) through (\d{1,3},\d{1,3})/).filter((str) => str)
    let [x0, y0] = fromXY.split(',');
    let [x1, y1] = toXY.split(',');
    x0 = parseInt(x0)
    x1 = parseInt(x1)
    y0 = parseInt(y0)
    y1 = parseInt(y1)
    ...
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        const cursor = y * 1000 + x;

        if (action === 'toggle') {
          lights[cursor] = !lights[cursor];
...
{ // Part 2
  const lights = Array(1000*1000).fill(0)
