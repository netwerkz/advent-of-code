...
{ // Part 1
    ...
    const key = (x, y) => `${x}:${y}`
    let prevConfiguration = {}
    for (let y = 0; y < input.length; y++) {
        for (let x = 0; x < input[0].length; x++) {
            prevConfiguration[key(x, y)] = input[y][x]
        }
    }
    ...
    const occupiedSeats = Object.values(prevConfiguration)
        .filter(el => el === '#')
        .length
    console.log(`[Part 1] result: ${occupiedSeats} `)
}
...
{ // Part 2
    ...
    const occupiedSeats = Object.values(prevConfiguration)
        .filter(el => el === '#')
        .length
    console.log(`[Part 2] result: ${occupiedSeats} `)
}