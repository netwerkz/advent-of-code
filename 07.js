...
{ // Part 1
    ...
    // setup node net
    const net = {} // will hold all 
    for (const rule of input) { // instantiate nodes
        const color = rule.split('bags')[0].trim()
        net[color] = new Bag(color)
    }    
    for (const rule of input) { // link node net
        const color = rule.split('bags')[0].trim()
        const contains = rule.split('contain')[1].trim()
        const containedBags = contains.split(', ').filter((el) => !el.includes('no other bags'))
        for(const containedBagsRule of containedBags) {
            const subRule = containedBagsRule.replace('bags', '').replace('bag', '').replace('.', '').trim()
            const matches = subRule.match(/^(\d+)\s(.+)$/)
            const containedColor = matches[2]
            net[color].bags[containedColor] = net[containedColor]
        }
    }

    // implement algorithm
    let counter = 0
    const traversedColors = []
    const doStep = (bagToBeContained) => {
        for(const color of Object.keys(net)) { // iterate over all bags
            if(traversedColors.includes(color)) continue // skip if we already have traversed this bag

            // if `bag` can contain `bagToBeContained`, add it to the traversedColors and increment counter
            const bag = net[color]
            if(bag.bags[bagToBeContained.color]) {
                traversedColors.push(color)
                counter ++
                doStep(bag) // recurse to advance algorithm step
            }
        }
    }

    doStep(net['shiny gold']) // start algorithm
    console.log('[Part 1] Counter: ', counter)
}
...
{ // Part 2
    ...
    for (const rule of input) { // link node net
        ...
        for(const containedBagsRule of containedBags) {
            ...
            const containedColor = matches[2]
            net[color].bags[containedColor] = net[containedColor]
            net[color].bagsCount[containedColor] = amount
        }
    }

    // implement algorithm
    let accumulator = 0
    const doStep = (currentBag, quantity) => {
        Object.keys(currentBag.bags)
            .map(color => {
                accumulator += parseInt(currentBag.bagsCount[color]) * quantity
                return color
            }) // accumulate quantity multiplied by count value
            .map(color => doStep(currentBag.bags[color], currentBag.bagsCount[color] * quantity))  // recurse to advance algorithm step
    }

    doStep(net['shiny gold'], 1) // start algorithm
    console.log('[Part 2] Counter: ', accumulator)
}