...
{ // Part 1
    ...
    const rotateVector = (x, y, degrees) => {
        ...
        const xx = Math.round(x * Math.cos(theta) - y * Math.sin(theta))
        const yy = Math.round(x * Math.sin(theta) + y * Math.cos(theta))
        return [xx, yy]
    }
    ...
    for (const cmd of input) {
        const regex = /^(?<letter>\w)(?<units>\d+)$/
        const matches = cmd.match(regex)
        let { letter, units } = matches.groups
        units = parseInt(units)
        switch (letter) {
            case 'L':
                [dirX, dirY] = rotateVector(dirX, dirY, units)
                break
            case 'R':
                [dirX, dirY] = rotateVector(dirX, dirY, -units)
                break
    ...
    console.log('[Part 1] x+y: ', Math.abs(posX) + Math.abs(posY))
}
...
const Part2 = (input, expected) => {    
    ...
    for (const cmd of input) {
        ...
        switch (letter) {
            case 'L':
                [waypointX, waypointY] = rotateVector(waypointX, waypointY, units)
                break
            case 'R':
                [waypointX, waypointY] = rotateVector(waypointX, waypointY, -units)
                break
            ...
            case 'W':
                ...
                break;
            case 'F':
                posX += units * waypointX
                posY += units * waypointY
                break;
        }
    }
    console.log(`[Part 2] X:${posX} Y:${posY} X+Y:${Math.abs(posX) + Math.abs(posY)}, expected ${expected}`)
}

Part2(sampleInput, 286)
Part2(input, '?')
