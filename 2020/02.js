...
{ // Part 1
    ...
    for(const value of input) {
        ...
        const [minmax, character] = policy.split(' ')
        
        const charOccurence = (password.match(new RegExp(character, "g")) || []).length
        const [minStr, maxStr] = minmax.split('-')
        const min = parseInt(minStr)
        const max = parseInt(maxStr)
        if(charOccurence >= min && charOccurence <= max) {
            countValidPasswords++
        }
    }
    console.log(`[Part 1] Found ${countValidPasswords} valid passwords`)
}
...
{ // Part 2
    ...
    for(const value of input) {
        const [policy, password] = value.split(': ')
        const [indexes, character] = policy.split(' ')
        
        const [index1Str, index2Str] = indexes.split('-')
        const index1 = parseInt(index1Str)-1
        const index2 = parseInt(index2Str)-1
        const char1 = password[index1]
        const char2 = password[index2]
        const char1match = char1 === character
        const char2match = char2 === character
        if( char1match ? !char2match : char2match ) { // http://www.howtocreate.co.uk/xor.html
            countValidPasswords++
        }
    }
    console.log(`[Part 2] Found ${countValidPasswords} valid passwords`)
}
